...
ll func(ll n,ll m){
    ...
    for(int i=0;i<primes.size();i++) {
          ...
          if(n%primes[i]==0){
            ...
            ///For example, the sum of all of the factors of
            /// 120=2^3*.3.5= (2^0+2^1+2^2+2^3)*(3^0+3^1)*(5^0+5^1)
            /// power series formula sn=(a*(r^n-1))/(r-1)
            /// r=primes[i] ,,,pw=cnt*m+1-> no of term ,,a=1;
            ll pw=cnt*m+1,r=primes[i];

            ll up=(modPow(r,pw)-1LL)%MOD;
            ll dn=modInverse(r-1);
            ll s=modMul(up,dn);
            sum=modMul(sum,s);
          }
    }
    ...
    if(n>2){
        ...
        if(n%MOD==0){
            ...
            ll up=(modPow(n,pod)-1LL)%MOD;
            ll dn=modInverse(n-1);
            ll s=modMul(up,dn);
            sum=modMul(sum,s);
        }
    }
